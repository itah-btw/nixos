...
_: {
  flake.nixosModules.tv-display =
    { pkgs, ... }:
    {
      ...
      # say so. boot.kernelPackages is linuxPackages_latest, so a new mainline
      # kernel arrives on every update. Never manages kwinoutputconfig.json --
      # KWin owns that. No-op when the mode is already right or the TV is off.
      systemd.user.services.tv-display-mode-guard =
        let
          script = pkgs.writeShellApplication {
            ...
            runtimeInputs = [
              ...
              # kdePackages.kscreen there is no kscreen-doctor on PATH and the
              # guard fails open while still logging success.
            ...
            text = ''
              ...
              # Log lines below interpolate $wantName only, never
              # $wantName@$wantRate: wantName is a kscreen mode name and already
              # carries its own rate, so the pair printed "1920x1080@60@60".
              # wantRate is a separate number only because the JSON refreshRate
              # comparison needs it.
              case "$state" in
                ok)
        ...
        in
        {
          ...
          # pulled in on Plasma, while the user manager always starts.
          serviceConfig = {
            Type = "oneshot";
      ...
      systemd.user.timers.tv-display-mode-guard = {
        ...
        # the user manager's timers.target is active -- the user manager starts
        ...
        timerConfig = {
          ...
          OnStartupSec = "30s";
          # Then keep checking: HDMI renegotiates on its own (power-cycling the
          # TV, an AVR handshake) and the guard only catches that if it looks
          # again. A correct display costs one JSON query and no flicker,
          # because kscreen-doctor is only called when the mode is wrong.
          OnUnitActiveSec = "2h";
          AccuracySec = "10s";
